...
class List extends Component {
  ...
  render() {
    const {toggleDeletePopup, deletePopup, deleteBook, haveBooks, bookList, popId, showEditBook, colorTags, match} = this.props;
            ...
            {bookList.map(item => {
              ...
              return (
                <li key={item.id} id={bookItemId} className={`book__list ${this.applyClass(bookItemId)}`}>
                  <CardDetail showEditBook={showEditBook} bookItemId={bookItemId} popId={popId} toggleDeletePopup={toggleDeletePopup} deletePopup={deletePopup} deleteBook={deleteBook} item={item} colorTags={colorTags} match={match}/>
                </li>
              )
            })}
          </ul>
