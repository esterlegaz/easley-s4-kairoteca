...
class List extends Component {
  ...
  render() {
    const {toggleDeletePopup, deletePopup, deleteBook, haveBooks, bookList, popId, colorTags} = this.props;
            ...
            {bookList.map(item => {
              ...
              return (
                <li key={item.id} id={bookItemId} className={`book__list ${this.applyClass(bookItemId)}`}>
                  <CardDetail bookItemId={bookItemId} popId={popId} toggleDeletePopup={toggleDeletePopup} deletePopup={deletePopup} deleteBook={deleteBook} item={item} colorTags={colorTags}/>
                </li>
              )
            })}
          </ul>
